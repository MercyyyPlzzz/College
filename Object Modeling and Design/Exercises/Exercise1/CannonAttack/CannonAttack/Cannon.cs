...
namespace CannonAttack
{
    public sealed class Cannon
    {
        ...
        public string ID
        {
            get
            {
                return (String.IsNullOrEmpty(CannonID)) ? CANNONID : CannonID ;
            }
            set
            {
                CannonID = value;
        ...
        private static Cannon cannonSingletInstance;
        ...
        public static Cannon GetInstance()
        {
            lock (padlock)
            {
                if (cannonSingletInstance == null)
                {
                    cannonSingletInstance = new Cannon();
                }
                return cannonSingletInstance;
            }
        }

        //Changes were made in "Shoot", but not sure if it's right
        ...
        public int CalculateDistanceOfCannonShot(int angle, int velocity)
        {
            ...
            double height = 0;
            double distance = 0;
            double angleInRadians = (3.1415926536 / 100) * angle;
            while(height>=0)
            {
                time++;
